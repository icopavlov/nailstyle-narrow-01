...
(function() {
	let expandableSections = $( ".section.expandable" );
	
	expandableSections.each( function() {
		let section = $( this ), 
			sectionHeaderHeight = section.find( ".header" ).outerHeight(),
			sectionMainHeight = section.find( ".main" ).outerHeight();
			
		section.get( 0 ).style.setProperty( "--section-header-height", sectionHeaderHeight + "px" );
		section.get( 0 ).style.setProperty( "--section-main-height", sectionMainHeight + "px" );
		
	} );
	
	expandableSections.on( "click", function( e ) {
		let target = $( e.target ), 
			section = $( this );
...
function markNav( nav ) {
	...
	if ( navWidth > windowWidth ) {
		nav.addClass( "rearrange" );
	} else {
		nav.removeClass( "rearrange" );
	}
} */
...
$( document ).ready( function() {
	...
	/* $( "header" ).on( "click", function( e ) {
		let target = $( e.target );
		if ( target.is( "#main-nav .button.open-nav" ) || target.is( "#main-nav .button.close-nav" ) ) {
			toggle( mainNav );
	...
	$( "header" ).on( "click", function( e ) {
		let target = $( e.target );
		if ( target.is( "#main-nav .button.open-nav" ) || target.is( ".scroll-to-top" ) ) {
			mainNav.removeClass( "close" );
			mainNav.addClass( "open" );
			// wasNavOpen = !wasNavOpen;
		} else if ( target.is( "#main-nav .button.close-nav" ) ) {
...
$( document ).ready( function() {
	// $( "#quick-contact" ).prependTo( "#slider .slide" );
	
	$( "#slider" ).get( 0 ).style.setProperty( "--slider-controls-outer-height", $( "#slider .controls" ).outerHeight() + "px" );
...
//	serviceGroups.slick( servicesContentOptions );
	...
	serviceGroups.on( "click", function( e ) {
		...
		if ( target.is( ".header .button.open" ) ) {
			openSection( targetServiceGroup );
		} else if ( target.is( ".main > .button.close" ) ) {
			closeSection( targetServiceGroup );
		}
...
$( document ).ready( function() {
	let formFields = $( "#send-email form" ).children( ".field" );
