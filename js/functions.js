...
function getCurrentSlide( slider ) {
	return slider.find( ".slide" ).eq( slider.slick( "slickCurrentSlide" ) );
}

function toTargetWhenIsButton( clickTarget ) {
	let target = $( clickTarget );
	if ( target.is( ".button.go-to-section" ) ) {
	...
	$( "#services .content" ).on( "click", function( e ) {
		...
		if ( target.is( ".button.open-service" ) ) {
			if ( currentService !== null ) {
				currentService.removeClass( "open" );
				currentRow.removeClass( "service-open" );
			}
			currentService = target.closest( ".service" );
			currentRow = target.closest( ".row" );
			
			currentService.removeClass( "close" );
			currentService.addClass( "open" );
			
			currentRow.removeClass( "service-close" );
			currentRow.addClass( "service-open" );
		} else if ( target.is( ".button.close-service" ) ) {
			currentService.removeClass( "open" );
			currentService.addClass( "close" );
			
			currentRow.removeClass( "service-open" );
			currentRow.addClass( "service-close" );
...
function attachRandomAccessFunctionallity( slider ) {
	let randomAccess = slider.parent().find( ".random-access" );
...
function scrollToTopToggle( scrollToTopButton ) {
	...
	scrollToTopButton.on( "click", function() {
		clearTimeout( timer );
		scrollToTopButton.removeClass( "reveal" );
...
function reveal( element ) {
	...
	element.removeClass( SCROLL_TO_TOP_BUTTON_HIDDEN_CLASS );
	return setTimeout( function() {
		element.addClass( SCROLL_TO_TOP_BUTTON_HIDDEN_CLASS );
